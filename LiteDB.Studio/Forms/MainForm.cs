...
namespace LiteDB.Studio
{
    public partial class MainForm : Form
    {
        ...
        public MainForm(string filename)
        {
            ...
            GridResult.DoubleBuffered(true);
        ...
        private void LoadResult(TaskData data)
        {
            ...
            else
            {
                ...
                if (data.Exception != null)
                {
                    ...
                    GridResult.BindErrorMessage(data.Sql, data.Exception);
                }
                else if (data.Result != null)
                {
                    if (tabResult.SelectedTab == tabGrid && data.IsGridLoaded == false)
                    {
                        GridResult.BindBsonData(data);
        ...
        {
            var cell = GridResult.Rows[e.RowIndex].Cells[e.ColumnIndex];
        ...
        {
            var field = GridResult.Columns[e.ColumnIndex].HeaderText;
            var id = GridResult.Rows[e.RowIndex].Cells["_id"].Tag as BsonValue;
            var cell = GridResult.Rows[e.RowIndex].Cells[e.ColumnIndex];
        ...
        private void TabResult_Selected(object sender, TabControlEventArgs e)
        {
            ...
            // set focus to result
            this.ActiveControl =
                tabResult.SelectedTab == tabGrid ? (Control)GridResult :
        ...
        private void GrdResult_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            ...
            switch (value.Type)
            {
                ...
                case BsonType.Guid:
                    e.Value = System.Text.RegularExpressions.Regex.Unescape(value.ToString());
                    break;

                default:
                    e.Value = System.Text.RegularExpressions.Regex.Unescape(JsonSerializer.Serialize(value));
                    break;
            }
        }
        ...
        {
            var bytes = System.Text.Encoding.UTF8.GetBytes(from);
